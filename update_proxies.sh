...
# 3. Regenerate 3proxy config (parent section only)
echo "[2/4] Regenerating 3proxy config..."
...
if [ "$COUNT" -eq 0 ]; then
  echo "❌ No valid proxies loaded – abort"
  exit 1
fi
...
echo "[4/4] Done!"
echo "✅ Proxy list refreshed & service restarted"
